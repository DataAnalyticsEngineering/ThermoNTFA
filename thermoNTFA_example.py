...
ntfa_material = ThermoMechNTFA(file_name='ms9p_fix_ntfa16_B1-6_10s_N24.h5', group_name='/', sig_y=my_sig_y, N_max=24)
    ...
    for iload in range(6):
        ...
        for theta in theta_list:
            ...
            eps_initial, sig_initial, C_initial, q_initial, xi_initial = \
                ntfa_material.UMAT_mixed(eps_idx=None, eps_n=np.zeros(6), deps=np.zeros(6), sig_bc=np.zeros(6),
                                         theta=theta, q_n=0., xi_n=np.zeros(ntfa_material.n_modes))
            print("q_ini: ", q_initial, eps_initial - eps_th_el)

            # eps = eps + eps_th_el[None, :]
            eps = eps + eps_initial
            # ntfa_material.A@eps[0] + ntfa_material.t_xi,
            # print("tau_initial: ",  np.linalg.norm( ntfa_material.A@eps[0] + ntfa_material.t_xi), ";  sig_f: ", ntfa_material.sig_y(theta,0.,False) * np.sqrt(2./3.) * ntfa_material.v_frac[0])
            # print("before: ", eps[0])
            xi = np.zeros(ntfa_material.n_modes)
            q = 0.
            C = np.zeros((6, 6))
        ...
        for theta in theta_list:
            ...
            for i in range(eps.shape[0]):
                ...
                if i < n_ramp:
                    ...
                    eps_idx = np.array([iload, ])
                eps[i], sig[i], C, q[i], xi[i] = \
                    ntfa_material.UMAT_mixed(eps_idx=eps_idx, eps_n=eps_n, deps=deps, sig_bc=np.zeros(6), theta=t,
                                             q_n=q_n, xi_n=xi_n)
                xi_n = xi[i].copy()
                q_n = q[i]
...
with h5py.File(os.path.join("data", "ms9p_uniaxial_stress_data_mod_loading0.h5"), "r") as F:
